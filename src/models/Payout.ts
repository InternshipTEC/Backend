...
export class Payout {
  ...
  @JoinColumn([{ name: 'bundle_currency', referencedColumnName: 'currencyCode' }])
  bundleCurrency2: Currency

  @ManyToOne(
    () => Currency,
    currency => currency.payouts2,
    {
      onDelete: 'RESTRICT',
      onUpdate: 'RESTRICT',
    },
  )
  @JoinColumn([{ name: 'payout_currency', referencedColumnName: 'currencyCode' }])
