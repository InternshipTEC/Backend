...
const processData = async () => {
  ...
  const getAllUserResponse = await request(app)
    .get(`/user/all`)
    .send()
  const deleteUserResponse = await request(app).delete(`/user/${postUserResponse.body.id}`)
  const signUpResponse = await request(app)
    .post('/auth/signup')
    .send(requests.signUpRequest)
  const loginResponse = await request(app)
    .post('/auth/login')
    .send({ name: postUserResponse.body.email, password: postUserResponse.body.password })
